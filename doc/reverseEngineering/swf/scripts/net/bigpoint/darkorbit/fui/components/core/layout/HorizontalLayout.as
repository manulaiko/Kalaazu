...
public class HorizontalLayout extends AbstractAlignmentLayout implements ILayout {
    ...
    protected function alignmentActivator(param1: Coreponent): void {
        param1.setAlignmentActivation(Alignment.ACIVATE_HORIZONTAL, false);
        param1.setAlignmentActivation(Alignment.ACIVATE_VERTICAL, true);
    }
    ...
    protected function calculateChildrenSizes(): void {
        ...
        if (_loc4_ = container as IContainer) {
            _loc5_ = Number((container as IContainer).getCalculatedWidth());
        } else {
            _loc5_ = 0;
        }
        var _loc6_: *;
        ...
        _loc2_ = 0;
        while (_loc2_ < _loc1_) {
            _loc3_ += absoluteChildren[_loc2_].width;
            _loc2_++;
        }
        ...
        _loc2_ = 0;
        while (_loc2_ < _loc1_) {
            _loc3_ += (relativeChildren[_loc2_] as ICoreponent).relativeWidth * remainingSizeAfterAbsolute;
            _loc2_++;
        }
    ...
    protected function drawLayout(): void {
        ...
        var _loc1_: Number = 0;
        var _loc3_: int = 0;
        for (; _loc3_ < childCount; _loc1_ += !!_loc2_ ? _loc2_.marginLeft : 0, child.x = _loc1_, _loc1_ += child.width, _loc1_ += _gap, _loc1_ += !!_loc2_ ? _loc2_.marginRight : 0, _loc3_++) {
            ...
            switch (_loc4_.sizeHandling) {
                case Coreponent.SIZE_RELATIVE:
                    _loc5_ = _loc4_.relativeWidth * remainingSizeAfterAbsolute;
                    _loc4_.width = _loc5_;
                    _loc2_.invalidate(InvalidationType.SIZE);
                    break;
                case Coreponent.SIZE_FILL:
                    _loc4_.width = fillElementSize;
                    _loc2_.invalidate(InvalidationType.SIZE);
                    break;
            }
        }
